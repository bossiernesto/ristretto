...
class RequestHandler(asynchat.async_chat, SimpleHTTPServer.SimpleHTTPRequestHandler):
    ...
    def handle_post_data(self):
        ...
        self.rfile = cStringIO.StringIO(''.join(deque.popall(self.incoming)))
        self.rfile.seek(0)
        self.do_POST()
    ...
    def do_GET(self):
        ...
        if qspos>=0:
            self.body = urlparse.parse_qs(self.path[qspos+1:], keep_blank_values=1)
    ...
    def do_POST(self):
        ...
        if ctype == 'multipart/form-data':
            ...
            self.body = urlparse.parse_qs(qs, keep_blank_values=1)
        else:
